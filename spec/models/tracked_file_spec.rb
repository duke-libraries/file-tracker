...
RSpec.describe TrackedFile do
  ...
  describe "save" do
    describe "SHA1 generation" do
      describe "when SHA1 is present" do
        subject { described_class.create(tracked_directory: dir, path: path, sha1: sha1) }
  ...
  describe ".track!" do
    ...
    let(:path3) { Tempfile.create("file-", dir.path).path }
    before do
      File.open(path1, "wb") { |f| f.write(SecureRandom.gen_random(1000)) }
      described_class.create(tracked_directory: dir, path: path1, size: 1000)
    ...
    it "checks the size of each previously tracked file path" do
      ...
      described_class.track!(dir, path1, path2, path3)
    end
    it "creates tracked files for new paths" do
      expect { described_class.track!(dir, path1, path2, path3) }.to change(TrackedFile, :count).by(2)
    end
  end
  ...
  describe "validation" do
    ...
    describe "with a symlink" do
      ...
      let(:dir) { TrackedDirectory.create(path: Dir.mktmpdir) }
      let(:path) { File.join(dir.path, "nypl.jpg") }
    ...
    describe "with an existing file" do
      ...
      describe "that is not unique" do
        before { described_class.create!(tracked_directory: dir, path: path) }
        it { is_expected.to be_invalid }
      end
  ...
  describe "track!" do
    ...
    describe "with an existing record" do
      subject { described_class.create!(tracked_directory: dir, path: path, size: size) }
  ...
  describe "checking fixity" do
    subject {
      described_class.create!(tracked_directory: dir, path: path, size: size, sha1: sha1)
    }
